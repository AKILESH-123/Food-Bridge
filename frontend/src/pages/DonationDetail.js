...
import { calculateFoodSafetyStatus, formatDurationMs, formatElapsedHuman } from '../utils/foodSafety';
            ...
            <div className="lg:col-span-3 space-y-5">
                      ...
                      {images.map((img, i) => (
                        <button key={i} onClick={() => setSelectedImage(i)}>
                          <img
                            ...
                            alt=""
                            className={`w-14 h-14 object-cover rounded-lg border-2 transition-all ${selectedImage === i ? 'border-green-500' : 'border-transparent'
                              }`}
                          />
                        </button>
                      ))}
                      ...
                      {steps.map((step, idx) => {
                        ...
                        return (
                          <div key={step.key} className="flex flex-col items-center text-center">
                            <div
                              className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold transition-all ${isPastStep
              ...
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
                ...
                {(() => {
                  ...
                  return (
                    <div className={`rounded-2xl p-5 border mb-5 ${isSafe ? 'bg-emerald-50/70 border-emerald-200 text-emerald-950' :
                      ...
                      <div className="w-full h-2.5 bg-gray-200 rounded-full overflow-hidden mb-2">
                        <div
                          className={`h-full rounded-full transition-all duration-300 ${isSafe ? 'bg-emerald-500' : isUrgent ? 'bg-amber-500' : 'bg-red-500'
                            }`}
                    ...
                    {isNGO && donation.status === 'available' && !requestedByMe && (() => {
                      ...
                      return isRed ? (
                        ...
                        <button
                          ...
                          className={`w-full py-3 rounded-xl font-bold text-sm text-white shadow-md transition-all ${safety.color === 'yellow' ? 'bg-amber-600 hover:bg-amber-700' : 'btn-primary'
                            }`}
                        >
